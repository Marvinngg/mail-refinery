...
{"updated_summary": "更新后的概要", "new_entries": "### YYYY-MM-DD — 发件人 [收件/发件]\\n内容..."}`;
  ...
  // 解析模型输出：先尝试 JSON，失败则按 markdown 提取
  let updatedSummary = null;
  let newEntries = null;

  try {
    const result = require('./model').parseJSON(response);
    updatedSummary = result.updated_summary;
    newEntries = result.new_entries;
  } catch {
    // 不是 JSON，按 markdown 处理
    // 尝试提取概要（"## 概要" 到下一个 "##" 之间）
    const summaryExtract = response.match(/##\s*概要\s*\n([\s\S]*?)(?=\n##|$)/);
    if (summaryExtract) updatedSummary = summaryExtract[1].trim();

    // 尝试提取时间线条目（### 开头的内容）
    const entriesExtract = response.match(/(###[\s\S]*)/);
    if (entriesExtract) {
      newEntries = entriesExtract[1].trim();
    } else {
      // 整段当作时间线条目
      newEntries = response.trim();
    }
  }

  // 更新概要
  let updatedMd = existingMd;
  if (updatedSummary && summaryMatch) {
    updatedMd = updatedMd.replace(summaryMatch[1], updatedSummary);
  }
